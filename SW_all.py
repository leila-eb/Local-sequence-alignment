#SW algo that returns all possible alignments retrived from the maximum value of the scores matrix
...
def sw_backtrack(seq1,seq2,scores,bt):
	...
	for start_i,start_j in d.keys():
		...
		while bt[i][j] != '':  #while we are not in the stop character
			if bt[i][j] == 'd': #we are matching
				...
				aln2 += seq2[i-1]
				i -= 1 #move upper
		d[start_i,start_j]= aln1[::-1], aln2[::-1]
...
d,score= sw_backtrack(seq1,seq2,scores,bt)

print('Score:',score) 
for aln1,aln2 in d.values():
	print(aln1)
	print(aln2)
	print()
